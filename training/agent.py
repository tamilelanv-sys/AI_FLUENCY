...
def agent(question, max_steps=6, verbose=True):
    ...
    for step in range(1, max_steps + 1):
        ...
        response = client.chat.completions.create(
            model=MODEL, messages=messages, tools=TOOLS, temperature=0, extra_body={"disable_tool_validation": True})
        ...
        for call in message.tool_calls:
            ...
            name = raw_name.split("<|channel|>")[0].strip()

            print("RAW TOOL NAME:", repr(raw_name))
            print("NORMALIZED TOOL NAME:", repr(name))
